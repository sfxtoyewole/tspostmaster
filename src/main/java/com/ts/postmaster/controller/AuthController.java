...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ApiResp<AuthDto> signUp(@RequestBody @Valid SignUpRequest signUpRequest) {
    ...
    @PostMapping("/signin")
    public ResponseEntity<?> sigIn(@RequestBody @Valid SignInReq signInReq) {
        var resp = userService.signIn(signInReq);
        HttpHeaders headers = new HttpHeaders();

        // Add one or more headers using the HttpHeaders methods
        headers.add("Authorization", resp.getData().getToken());
